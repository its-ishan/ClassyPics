...
class Unet(nn.Module):
    ...
    def get_activation(self, x, t, cond_input=None):
        model = self
        activations = []
        activation_fn = nn.SiLU()  # Assuming SiLU activation is used in the model
        ...
        if model.image_cond:
            ...
            im_cond = cond_input['image']
            im_cond = torch.nn.functional.interpolate(im_cond, size=x.shape[-2:])
            im_cond = model.cond_conv_in(im_cond)
            assert im_cond.shape[-2:] == x.shape[-2:]
            x = torch.cat([x, im_cond], dim=1)
            out = model.conv_in_concat(x)
        else:
            out = model.conv_in(x)

        # t_emb -> B x t_emb_dim
        t_emb = get_time_embedding(torch.as_tensor(t).long(), model.t_emb_dim)
        t_emb = model.t_proj(t_emb)

        if model.class_cond:
            validate_class_conditional_input(cond_input, x, model.num_classes)
            class_embed = einsum(cond_input['class'].float(), model.class_emb.weight, 'b n, n d -> b d')
            t_emb += class_embed
        ...
        for idx, down in enumerate(model.downs):
            out = down(out, t_emb, context_hidden_states)
            activations.append(activation_fn(out))
        ...
        for up in model.ups:
            down_out = activations.pop()
            out = up(out, down_out, t_emb, context_hidden_states)
            activations.append(activation_fn(out))

        out = model.norm_out(out)
